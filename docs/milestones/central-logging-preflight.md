...
The accepted Loki allocation is a new 8 GiB partition `p3` beginning at sector `75499520`, length `16777216` sectors, ending at `92276735`. The next sector, `92276736`, remains unallocated. This allocates less than two percent of the previously reported unused extent. A static 8 GiB PV and reserved PVC will reference `/mnt/signalforge-loki/data`, bind only on `forge-head`, and use `Retain`. The partition, rather than the PV's nominal capacity, enforces the actual filesystem ceiling. Seven-day retention is a policy target; disk alerts and a tested backup are still required.

### Storage preparation, 2026-09-23

The operator compared the reviewed script's SHA-256 `c11448fb6a383e60f823b64787b89fd84ef9c0d822438bbbc8ce16b1937a3b74` with the copy on `forge-head`. `--plan` showed the exact `p1`/`p2` layout and a simulated `p3` at sectors `75499520`–`92276735`; `sfdisk` reported the table unchanged. The subsequent `--prepare-without-offnode-backup` completed without errors. It reported a new ext4 filesystem UUID `93a19402-4a5b-4689-aed7-f1841c2cb53b` and host evidence at `/var/tmp/signalforge-loki-storage.8EDDMf`. The script includes checks that old partition records remain identical, mounts by UUID, verifies UID/GID `10001:10001` can write, remounts, and verifies fstab. The operator then independently observed `findmnt` reporting `/dev/nvme0n1p3` mounted read/write as ext4 at that UUID; `lsblk` showed the separate 32 GiB Prometheus and 4 GiB Grafana partitions still mounted at their expected UUIDs. `/mnt/signalforge-loki/data` is owned by `10001:10001` with mode `750`; `df` showed 7.8 GiB total, 2.1 MiB used and 7.4 GiB available. No Kubernetes object was created by the storage script.
