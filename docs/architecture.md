...
- Deployment: one stateless replica using immutable release `0.5.0`; local source `0.5.1` corrects finding-navigation scrolling and is not yet published or staged for deployment
...
1. Publish and deploy the separately approved immutable `0.5.1` navigation correction, then repeat browser acceptance before PR readiness and merge.
