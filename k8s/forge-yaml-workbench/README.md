...
The currently accepted live `0.1.2` deployment has one available Ready replica with zero restarts and a runtime ImageID matching OCI index digest `sha256:07f34be33c55bca5b7bf5321e5d149e4831ce520c9efbdd98233468a1a50e3c7`. NodePort `30081`, its ready EndpointSlice, `/healthz`, the application page, security headers, sample loading, format feedback, editing state, the format shortcut, and YAML download passed live acceptance on 2026-09-12. The manifest now pins the separately published `0.2.0` image; deployment still requires server-side dry-run, diff review, and explicit approval.
