...
This directory contains the restricted `forge-tools` namespace, hardened stateless Workbench Deployment, and NodePort Service on `30081`. The workload uses the immutable `0.1.0` multi-architecture image index, has no Kubernetes API identity or mounted ServiceAccount token, and processes pasted YAML only in the browser.
