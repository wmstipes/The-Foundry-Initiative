...
This directory contains the restricted `forge-tools` namespace, hardened stateless Workbench Deployment, and NodePort Service on `30081`. The manifest contains the digest-pinned `0.3.0` deployment candidate while the live workload remains on immutable `0.2.0`. The Workbench has no Kubernetes API identity or mounted ServiceAccount token and processes pasted YAML only in the browser.
