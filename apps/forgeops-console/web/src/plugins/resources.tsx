...
export default function ResourceBrowser({ scope, kind, result, selected, busy, history, onKind, onRefresh, onSelect, onPodTarget }: Props) {
  return <section className="resource-shell panel">
    ...
    <div className="resource-main">
      <div className="resource-heading"><div><p className="eyebrow">Resource explorer</p><h2>{kinds.find((item) => item.id === kind)?.label}</h2></div><div className="resource-actions"><details className="activity-panel"><summary aria-label={`Activity · Session requests (${history.length > 8 ? `latest 8 of ${history.length}` : history.length})`}>Activity <span>({history.length > 8 ? `8 of ${history.length}` : history.length})</span></summary><div className="activity-content"><h3>Session requests</h3>{history.length === 0 ? <p className="empty-state">No resource activity yet.</p> : <ol>{history.slice(-8).reverse().map((entry) => <li key={entry.sequence}><strong>{entry.outcome}</strong><span>{entry.context}/{entry.namespace || "cluster"}</span><small>gen {entry.generation} · {entry.itemCount} items</small></li>)}</ol>}</div></details><button type="button" disabled={busy || !scope.context || (kind !== "nodes" && !scope.namespace)} onClick={onRefresh}>{busy ? "Loading…" : "Refresh"}</button></div></div>
...
function Relations({ title, records, onPodTarget, busy }: { title: string; records: ResourceRecord["related"]; onPodTarget?: (name: string, uid?: string) => void; busy?: boolean }) {
  return <div className="relations"><h4>{title}</h4>{records.map((record, index) => <p key={`${record.relation}/${record.kind}/${record.name}/${index}`}><span className="relation-text"><span className="relation-label">{record.relation}</span> {record.kind}/{record.name}</span>{record.kind === "Pod" && record.relation === "endpoint-target" && onPodTarget && <button type="button" disabled={busy} onClick={() => onPodTarget(record.name, record.uid)} aria-label={`Inspect Pod ${record.name}${record.uid ? "" : " by name only"}`}>Inspect Pod{record.uid ? "" : " (name only)"}</button>}</p>)}</div>;
}
