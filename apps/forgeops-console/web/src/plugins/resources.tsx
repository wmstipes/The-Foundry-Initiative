...
interface Props {
  ...
  busy: boolean; history: ActivityEntry[]; onKind(kind: ResourceKind): void; onRefresh(): void; onSelect(record: ResourceRecord): void; onPodTarget(name: string): void;
}
...
function Relations({ title, records, onPodTarget, busy }: { title: string; records: ResourceRecord["related"]; onPodTarget?: (name: string) => void; busy?: boolean }) {
  return <div className="relations"><h4>{title}</h4>{records.map((record) => <p key={`${record.relation}/${record.kind}/${record.name}`}><span>{record.relation}</span> {record.kind}/{record.name}{record.kind === "Pod" && record.relation === "endpoint-target" && onPodTarget && <button type="button" disabled={busy} onClick={() => onPodTarget(record.name)} aria-label={`Inspect Pod ${record.name}`}>Inspect Pod</button>}</p>)}</div>;
}
