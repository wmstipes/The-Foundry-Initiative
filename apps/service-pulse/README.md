...
The proposed Kubernetes manifests in `k8s/service-pulse` use the verified index digest. The separate `forge-pulse` namespace contains only two one-replica Deployments and ClusterIP Services, with no ServiceAccount token, non-root/read-only filesystem, resource requests/limits, readiness/liveness, port-forward-only board access and scoped rollback. See its README for the read-only preflight and phased review. Check actual node capacity and confirm the Restaurant API Service DNS/port on the live cluster before deployment. Merging manifests does not apply cluster objects.
