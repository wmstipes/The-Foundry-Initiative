...
This first PR prepares source and a container recipe. **No image is published and no cluster objects are changed.** The next gate builds and verifies an arm64 image and its digest before drafting Kubernetes manifests. The manifest review must include: a separate `forge-pulse` namespace; only two one-replica Deployments and ClusterIP Services; no ServiceAccount token; non-root/read-only filesystem; resource requests/limits; readiness/liveness; port-forward-only board access; exact image digest; and scoped rollback. Check actual node capacity and confirm the Restaurant API Service DNS/port before deployment. Dockerfile base image and dependency versions must be pinned to reviewed digests for the release candidate.
