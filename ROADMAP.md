...
The active workstream is SignalForge, a four-node Raspberry Pi Kubernetes lab. Milestones 001-030 and 032-036 are complete. Restaurant API `0.7.0` runs as three replicas alongside lightweight Prometheus, Kubernetes Metrics Server, Grafana, bounded rule evaluation, and Forge YAML Workbench. NVMe-backed Prometheus and Grafana use retained local storage with tested persistence and recovery procedures; the Workbench is intentionally stateless and performs analysis in the browser. Accepted Workbench `0.4.1` precompiles the pinned Kubernetes `v1.36.4` validators under the strict CSP. Milestone 037 is in local implementation.
...
**Status:** In progress locally. Publication, image publication, deployment, browser acceptance, and merge remain separate approval gates.
