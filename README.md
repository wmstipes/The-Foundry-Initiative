...
Milestone 036 source advances the Workbench to a local `0.4.0` candidate with schemas pinned to Kubernetes `v1.36.4`. It validates 12 explicitly supported GVKs entirely in the browser and reports unsupported built-ins, unavailable CRD schemas, and incomplete identities without validity claims. The deployed release remains `0.3.0` until the separate publication and deployment gates are approved.
