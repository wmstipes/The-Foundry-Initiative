...
Milestone 042 is locally implementing candidate `0.10.0`: one-file YAML drag-and-drop with visible and accessible target states, shared unsaved-change protection, deterministic state boundaries, and no upload or persistence path. Immutable `0.9.0` remains the tracked and live deployment until later publication, manifest, and deployment gates are separately approved.
