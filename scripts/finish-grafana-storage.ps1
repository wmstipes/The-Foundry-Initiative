...
if ($Partitions.label -ne 'gpt' -or $Partitions.id -ne 'FDF2C7FC-C23E-4147-93B5-B53D85A08F78' -or $Partitions.sectorsize -ne 512 -or @($Partitions.partitions).Count -ne 2) {
...
$Second = $Partitions.partitions[1]
if ($Second.node -ne '/dev/nvme0n1p2' -or $Second.start -ne 67110912 -or $Second.size -ne 8388608 -or $Second.uuid -ne 'A8E50BC1-1B9C-419B-A13D-3EE72C29FF56' -or $Second.name -ne 'signalforge-grafana') {
    throw 'Existing Grafana partition does not match the reviewed partial preparation'
}
Write-Host "Verified Prometheus archive and saved partition table off-node: $EvidenceDirectory"
Write-Host 'Existing allocation to verify: /dev/nvme0n1p2; start 67110912; size 8388608 sectors; end 75499519; 4 GiB.'
Write-Host 'Preserve /dev/nvme0n1p1, its UUID and active Prometheus mount.'
if (-not $Finish) {
    Write-Host 'Backup-only phase complete. Supply -Finish to format and mount the verified existing Grafana partition.'
    return
}
...
# Resume only: no partition-table writes and no kernel partition changes.
test -b "$part"
...
if ($HostExit -ne 0) { throw 'Storage completion stopped. Keep all evidence; do not rerun after a partial write. Return the output for review.' }
Write-Host "Storage completion evidence: $EvidenceDirectory"
